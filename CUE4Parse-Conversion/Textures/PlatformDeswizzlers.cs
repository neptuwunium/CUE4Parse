...
public static class PlatformDeswizzlers
{
    ...
    private static void PrepareDllFile()
    {
        ...
        var fileOk = false;
        var fileHash = Convert.ToHexString(SHA1.HashData(ba));

        if (File.Exists("tegra_swizzle.dll"))
        {
            var bb = File.ReadAllBytes("tegra_swizzle.dll");
            var fileHash2 = Convert.ToHexString(SHA1.HashData(bb));
        ...
        if (!fileOk)
        {
            File.WriteAllBytes("tegra_swizzle.dll", ba);
        }
    }
